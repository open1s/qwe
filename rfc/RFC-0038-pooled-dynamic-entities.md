...
Implementation is staged: **Stage 1** adds the `active` flag, `active_id()`, runtime read/write, snapshot/state-hash coverage, present hiding, and the `FindFreeSlot` opcode/runtime. **Stage 2** adds the `pool` declaration, slot layout, the active guard, and the `spawn`/`despawn`/`active` language surface. **Stage 3** re-baselines conformance and the examples.
