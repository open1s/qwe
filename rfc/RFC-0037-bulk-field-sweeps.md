...
6. **Compatibility.** Opcodes are additive; existing modules (per-cell lowering, codec, AOT artifacts) remain valid. `apply_writes` no longer mutates grid cells (the overlay flush owns that path); unknown/older runtimes that do not implement the bulk methods return an error (`Status::Internal`) and cannot execute bulk-opcode modules — the reference interpreter is the semantic oracle.

Implemented in three stages, all landed: **Stage 1** introduced the dense field overlay and routed scalar grid access through it with the `WorldWrite` list unchanged. **Stage 2** added the three opcodes, the single-instruction lowering, the overlay equality check, and the commit flush (in `LangRuntime::{step_interpreter,step_jit,step_cross}` and `Module::run_step`). **Stage 3** re-baselined conformance (17/0), all 11 example programs, and the 289+ tests.
